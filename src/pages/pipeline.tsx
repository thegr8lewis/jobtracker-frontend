...
const normalizeStatus = (status: string): ApplicationStatus => {
  ...
  const statusMapping: Record<ApplicationStatus, string[]> = {
    saved: ['saved', 'wishlist', 'to apply'],
    applied: ['applied', 'submitted', 'application sent'],
    interview: [
      'interview', 'interview scheduled', 'interview invite', 
      'interview confirmation', 'interview process', 'interview succeeded',
      'interview passed', 'interview completed', 'interview failed',
      'interview rejected', 'phone screen', 'technical interview',
      'behavioral interview', 'onsite interview'
    ],
    offer: [
      'offer', 'job offer', 'employment offer', 'congratulations',
      'offer extended', 'offer letter', 'offered'
    ],
    rejected: [
      'rejected', 'rejection', 'not selected', 'unsuccessful',
      'interview failed', 'interview rejected', 'position filled',
      'other candidate', 'declined', 'not moving forward'
    ],
    withdrawn: ['withdrawn', 'withdrew', 'cancelled']
  };
  
  // Check for matches
  for (const [standardStatus, variations] of Object.entries(statusMapping)) {
    if (variations.some(variation => statusStr.includes(variation))) {
      return standardStatus as ApplicationStatus;
    }
  }
...
const statusColumns: {
  ...
  },
  {
    key: "interview",
    label: "Interview",
    description: "All interview stages",
...
const PipelinePage: React.FC = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      // Normalize status for each application if needed
      const appsWithNormalizedStatus = (response.data || response).map((app: Application) => ({
        ...app,
        // Only normalize if the status seems to be an AI-generated string
        // and not already one of our standard statuses
        status: Object.keys(statusColumns).includes(app.status) 
          ? app.status as ApplicationStatus 
          : normalizeStatus(app.status)
      }))
      
      setApplications(appsWithNormalizedStatus)
    } catch (err: any) {
    ...
    <div className="flex flex-row items-center gap-4 sm:gap-6 w-full sm:w-auto">
      ...
      <div className="relative flex-1 max-w-xs sm:max-w-sm">
        ...
        <input
          ...
          className="pl-10 pr-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder-white/50 focus:ring-2 focus:ring-white/20 focus:border-white/30 transition-all backdrop-blur-sm w-full"
        />
      </div>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-all duration-200 border border-white/10"
      >
      ...
      <button
        onClick={handleAddApplication}
        className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white font-semibold transition-all duration-200 shadow-lg hover:shadow-purple-500/25"
      >
        <FiPlus className="w-5 h-5" />
        {/* Hide text on small screens, show on sm and above */}
